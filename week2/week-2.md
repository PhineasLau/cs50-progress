...
## Use headerfile of ctype.h to help manipulate string in c.
...
### To input a variable to an array type in, (array name)[position] = (value);.
...
### String is actually just an array of characters, and each characters takes 1 byte of memory as a letter, but also holds an ACSII value. Therefore you can output the ACSII value of each character in a string by using printf and %i and referring the positions of the characters in a string. And so you could compare characters like numbers as this will automatically refers to their ACSII value.
### String is declared with "", which so it adds an extra byte of memory to not just the variables but the null value, or \0. This ensure that the computer knows where the string ends. Therefore we can count the length of the string by using a while loop to count until it meets a character as '\0'. We could also do the same as it is basically the strlen function included in string.h. 

## main
### In many programming lanuages like c, the main would be the function that runs automatically when you compile and run your code. The syntax of int main(void) indicates that the function doesn't take in any command line arugement to start by the void. 
### However, the main function can actually take in arguement, such as int main(int argc, string argv[]), arg is arguement, you declare the amount of arguements as argc, arguement count. This variable counts from 1 like normal human, for the totol amount of words or arugements in the command line, including the ./(programname) itself. Therefore when you typed in ./(programname) Phineas, the argc would be 2. Whereas argv is the arguement vector, agrv counts from 0 as an array, therefore when you need to refer to an user input as they run the main, you need to use agrv[1] or more, as agrv[0] is the program name, which you can use it to self refer instead.
### As an convention of c, main can only either takes in void as its arguement or those two. They will be auomatically filled in, and you can edit the variable names.

## Reflection
### Is it really unpreventable for good programmers to reverse engineer out the original logic of the source code by recompiling? Would this be the reason why different companies uses different CPUs as it has different secert assembly codes to its lanuage so to prevent or create difficulty for recompiling?
#### Yes it is basically impossible to prevent someone from recompiling the code, as the code ultimately needs to be understandable by a computer. However, the structure, comments and variables names would be discarded, as compiler when converting source code to assembly code and to binary code it will automatically reorder and deletes contents that are for humans to understand better for optimisation for speed. Companies simply uses different CPUs as different do different jobs better, to increase the difficulty of recompiling, engineer instead uses obfuscation, adding decoy logics and scrambling the code structure. As so even recompilier recompile the code, it would takes much more time and effort for humans to figure out the true logic by understanding it.  

## Projects
### 1.scrabble.c
#### A program to give different words different points based on how rare each letters are across all english words. First time used the functions in ctype.h to save time of to transfer each characters of to its upper case form. Successfully used an improved way to refer and manipulate part of a given data from last project of credit.c, as understanding a string as an array of characters helps me understand that the way to extract a part of it is essentially same as to extract an element from an array. However, after finishing I realised that the code can actually be further improved to make it more easy to maintain, by using an array to store each letter's score instead of writing a long list to see if a letter match.
### 2.readability.c
#### A program to help indicate the reading level of a user given texts, struggled with the final output but as to then realised I mistaken used int instead of float for the function that calculate the index, preventing the rounding from start. In processes first time used the debug tool to help track variable changes to notice the actually error that is happening.
