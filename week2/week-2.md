...
### String is actually just an array of characters, and each characters takes 1 byte of memory as a letter, but also holds an ACSII value. Therefore you can output the ACSII value of each character in a string by using printf and %i and referring the posistions of the characters in a string. String is declared with "", which so it adds an extra byte of memory to not just the variables but the null value, or \0. This ensure that the computer knows where the string ends. 
