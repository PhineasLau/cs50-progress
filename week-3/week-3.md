# Week 3
## Recursion
### Function that is to call itself repeatedly to break down the larger to smaller problem, as a loop to repeat itself after got handed the base input, until a base condition is met, set by the line that returns a value. But repeating too many times may lead to error of too high memory usage as everytime calling a new version of itself increases memory needed whereas loops would be more complex but without this problem.
...
### Projects
#### 1. Plurality.c
##### Program that is to take in candidates name and voter amount, then to let each voter vote for one candidate of their choice and then to output the winner. There was a clean solution to this as loop through the list, remember the highest votes ever recorded, then to loop again and print all candidate with that amount of votes. However by that time I decided to practice and apply my knowledge of selection sort to it, to sort the candidates from least to most votes, then to loop from last index backward, for all candidates with same amount of votes as final index's to print them out. As selection sort guarantee that largest value always at last index. 
##### The first struggle I encountered is that I was overcorrecting the zero index idea in c, which I used j < candidate_count - 1 which j = i+1 for the loop, but in fact there is no need to -1, as it is < not <= so it will naturally be one index away, and even if j = i+1 is fine because for loop checks the condition first before running anything in the {}, including the first time the loop was executed, the fix is simply make candidate_count -1 to just candidate_count.
##### The second struggle I encountered contrasts the first one, my backward printing loop started at int i = candidate_count, which becuas of the zero indexing it starts one index forward the array, causing garbarge values to be printed. So I fixed it by starting it by candidate_count - 1. Overall for setting endpoint, < or > as long as without = is enough, but setting start point or referring to array should -1.
#### 2.tideman.c
##### It is for now by far the most difficult and complex programming projecct I have ever done. This is labelled as the only "very very very comfortable" option in cs50, while before it was just either less or more comfortable. I have picked this one to challenge myself as how algorithm design is strongly linked to my goal of becoming a cybersecurity engineer, yet this still out of my expect of the difficulty and take me some extra time for week 4 for it. I will detailedly go through the struggles I have been through. Based on how it is famously known as the most difficult project in cs50 and how large scale it is I will write the reflection of it separately in a new .md file. 
