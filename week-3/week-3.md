...
### Sometimes it could be wiser to trade off a level of effciency or to make a more expensive but informed decision knowingly for a specific case. Such as to avoid consuming long time to fine tune and to recorrect the new version of a program that supposed to be only ran once ever just to save a few seconds time. Though it would be not the case if the program is going to be repeatablly used.  
## Sorting Algorithms
### Linear search
