...
#### To start a new file, type in terminal, code <program name>.c, it is recommended to use lowercase and no space for the name.
#### To compile, translate the source code to runable machine code you type the following in terminal, make <program name>, with no need of typing in the format .c. No output should been seen if done correctly. You need to recompile the program everytime after you made a change in order for a change in output.
