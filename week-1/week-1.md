...
## You could overflow a variable if you assign too high value to it, as int can only have up to around 4 billion different values, so up and down to 2 billion for positive and negative, where as l[...]
...
## Flowing point imprecision may happen by when asking computer to output an infinitely repeating result like 1/3, which effect can be reduced by using more bits decimal data type like double inst[...]
## Machine codes is the only codes that computer understand, which is binary of 0s and 1s, and source code is what human writes, like C and python. That require translation by a compiler, such as [...]
...
#### To compile, translate the source code to runable machine code you type the following in terminal, make (programname), with no need of typing in the format .c. No output should been seen if do[...]
...
#### To gain access to many of the commands, including those comes with c like print, you need to include or import headerfile, that contain different libraries, that are codes pre written for dif[...]
...
#### Escape sequences are sequences that not read directly in string qualifiers, but served for specific functions, or to replace characters that cannot be output directly. \r for return to start [...]
#### To get a text input, use string (variable) = get_string("(text)");, which stores the string input to (variable). This code automatically uses the printf command as well to output (text). You [...]
#### When you declare a varaibale though an input, and the user input format doesn't match the specifyed format, it will loop again, such as char a = get_char(type: );, and user typed multiple cha[...]
#### For conditionals follow the structure:
#### if (condition)
#### {
#### (4 spaces for convention) command
#### }
#### While loops follow the same structure as well, for loops also but in the conditions you do (declare the starting point of the variable; ending condition; value increase after each loop), both[...]
#### You can also use the do command to perform lines of codes once first, then to check conditions to see if repeat. It follows the same structure as well, except the while conditions part are me[...]
#### For functions that takes no input and generate no output, but with side effect, use void (function_name)(void), then next line {(commands)}, if you want to take in or output a value, change b[...]
